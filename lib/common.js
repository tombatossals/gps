var Link    = require("../models/link"),
    ...
    NodePublicInfo = require("../models/node_public_info"),
    util      = require("util"),
...
var getNodesPublicInfoByName = function(nodeNames) {
    var deferred = Q.defer();
    ...
    NodePublicInfo.find(query, function(error, nodes) {
        if (error) {
            console.log(error);
            deferred.reject();
            return;
...
var getLinkById = function(linkId) {
    var deferred = Q.defer();
    query =  { "_id": ObjectId(linkId) };
    Link.find(query, function(error, link) {
        if (error) {
            console.log(error);
            deferred.reject();
            return;
        }
        deferred.resolve(link[0]);
    });
...
module.exports.getLinkById = getLinkById;
