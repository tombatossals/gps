...
function getips(supernodo) {
    var deferred = Q.defer();

    var ip = supernodo.mainip;
    var connection = new api(ip, supernodo.username, supernodo.password);
    connection.on('error', function(err) {
        logger.error(util.format("FATAL: Can't connect to the API: %s %s %s", ip, supernodo.username, supernodo.password));
        deferred.reject();
    });
        ...
        chan.write('/ip/address/print',function() {
            chan.on('done',function(data) {
                ...
                parsed.forEach(function(item) {
                    ...
                    count--;
                    if (count === 0) {
                        supernodo.interfaces = interfaces;
                        deferred.resolve(supernodo);
                    }
                });
                chan.close();
                conn.close();
